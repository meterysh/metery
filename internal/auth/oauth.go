...
func NewOAuthHandler(cfg OAuthConfig, sessions *SessionManager, st *store.Store) *OAuthHandler {
...
func (h *OAuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, h.cfg.AuthCodeURL(state, oauth2.AccessTypeOnline), http.StatusFound)
}

func (h *OAuthHandler) Callback(w http.ResponseWriter, r *http.Request) {
	want, err := r.Cookie("oauth_state")
	if err != nil || want.Value == "" {
		http.Error(w, "missing state", http.StatusBadRequest)
		return
	}
	if r.URL.Query().Get("state") != want.Value {
	...
	user, err := h.st.UpsertUser(r.Context(), info.ID, info.Email, info.Name)
	if err != nil {
